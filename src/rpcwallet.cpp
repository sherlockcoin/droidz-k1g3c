...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress <droidzaddress> <amount> [comment] [comment-to] [tx-comment]\n"
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom <fromaccount> <todroidzaddress> <amount> [minconf=1] [comment] [comment-to] [tx-comment]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    // droidz: if user OS account compromised prevent trivial sendmoney commands
    if (params.size() > 2)
...
// droidz: reserve balance from being staked for network protection
...
// droidz: make a public-private key pair
