...
static const unsigned int TX_COMMENT_HEIGHT = 256400;
...
public:
    ...
    {
        // droidz: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // droidz: get transaction coin age
...
public:
    ...
    // droidz: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // droidz: two types of block: proof-of-work or proof-of-stake
    ...
    // droidz: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // droidz: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // droidz: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // droidz: block index flags
    enum  
    {
